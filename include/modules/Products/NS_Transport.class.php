...
class Products_NS_Transport extends Products {
                ...
                'fields'     =>
                    array(
                        ...
                        array(
                            ...
                            'condition'             => 'product_types_id = 23',//ограничиваем продуктовым рядом нс
                            'display'               =>
                                array(
                                    'show'          => false,
                                    'insert'        => true,
    ...
    function Products_NS_Transport($data) {
        Form::Form($data);
    ...
    function showForm($data, $action, $actionType=null, $template=null) {
        return parent::showForm($data, $action, $actionType, 'ns_transport.php');
    }
    ...
    function calculate($risks, $financial_institutions_id, $price, $discount, $cart_discount, $agencies_id, $terms_id, &$data) {
        ...
        $conditions[] = 'a.product_types_id = 23';
        ...
        $sql =  'SELECT ' .
                ...
                'JOIN ' . PREFIX . '_products_ns_transport AS b ON a.id = b.products_id ' .
                'JOIN ' . PREFIX . '_product_risks AS c ON a.id = c.products_id ' .
                'JOIN ' . PREFIX . '_product_car_brand_assignments AS br ON b.products_id = br.products_id AND br.car_brands_id = ' . intval($data['brands_id']) . ' ' .
...
//_dump($row);
        ...
        if (!in_array($price,array(50000,100000,200000))) $data['product_factors']=0;
    ...
    function exportInWindow($data) {
        ...
        $sql    = 'select * from insurance_products a join insurance_products_ns_transport b on b.products_id=a.id ';
        $list   = $db->getAll($sql);
