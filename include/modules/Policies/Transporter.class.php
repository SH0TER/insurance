...
class Policies_Transporter extends Policies {
    ...
    function Policies_Transporter($data) {
        Policies::Policies($data);

        $this->objectTitle = 'Policies_Transporter';

        $this->messages['plural'] = 'Договори "Добровільного страхування відповідальності перевізника"';
        $this->messages['single'] = 'Договір "Добровільного страхування відповідальності перевізника"';
	...
	function showForm($data, $action, $actionType=null, $template=null) {
		...
		$sql =	'SELECT a.id, a.title ' .
				...
				'WHERE a.product_types_id = ' . PRODUCT_TYPES_TRANSPORTER . ' ' .
				'ORDER BY a.title';
	...
	function setAdditionalFields($id, $data) {
        ...
        $sql =  'UPDATE ' . PREFIX . '_policies AS a ' .
                'JOIN ' . PREFIX . '_policies_transporter AS b ON a.id = b.policies_id ' .
    ...
    function insert($data, $redirect=true, $showForm=true, $checkFieldsAndReturn=false) {
        ...
        if (intval($data['id'])) {
            ...
            if ($Authorization->data['roles_id'] == ROLES_AGENT) {
                ...
                $sql =  'UPDATE ' . PREFIX . '_policies_transporter SET ' .
	...
	function renewPolicy($data) {//алгоритм Переукласти поліс
		...
		$data['product_types_id']	= PRODUCT_TYPES_TRANSPORTER;
		$data['policy_statuses_id']	= POLICY_STATUSES_CREATED;
    ...
    function getXML($data) {
        ...
        return  $Smarty->fetch($this->object . '/transporter.xml');
    }
}
