...
class Policies_DangerObjects extends Policies {
                    ...
                    array(
                       ...
                       /*array(
                        ...
                        array(
                            ...
                            'type'              => fldText,
                            'maxlenght'         => 20,
							'validationRule'	=> '903.[0-9][0-9].[0-9]{6}',
                            'display'           =>
                                array(
                                    'show'      => true,
                                    'insert'    => true,
    ...
    function Policies_DangerObjects($data) {
        Policies::Policies($data);

        $this->objectTitle = 'Policies_Transporter';

        $this->messages['plural'] = 'Договори "Небезпечні об\'єкти"';
        $this->messages['single'] = 'Договір "Небезпечні об\'єкти"';
	...
	function showForm($data, $action, $actionType=null, $template=null) {
		...
		$sql =	'SELECT a.id, a.title ' .
				...
				'WHERE a.product_types_id = ' . PRODUCT_TYPES_DANGER_OBJECTS . ' ' .
				'ORDER BY a.title';
	...
	function setAdditionalFields($id, $data) {
        ...
        $sql =  'UPDATE ' . PREFIX . '_policies AS a ' .
                'JOIN ' . PREFIX . '_policies_danger_objects AS b ON a.id = b.policies_id ' .
    ...
    function insert($data, $redirect=true, $showForm=true, $checkFieldsAndReturn=false) {
        ...
        if (intval($data['id'])) {
            ...
            if ($Authorization->data['roles_id'] == ROLES_AGENT) {
                ...
                $sql =  'UPDATE ' . PREFIX . '_policies_danger_objects SET ' .
	...
	//Export 1C 7.7
     function getXML($data) {
      ...
      if ($data['number']) {
        ...
        return  $Smarty->fetch($this->object . '/danger_objects.xml');
    }
