<?
/*
 * Title: policy Policies_ThirdPartyLiability class
 *
...
class Policies_ThirdPartyLiability extends Policies {
                ...
                'fields'     =>
                    array(
						...
						array(
                            ...
                            'table'             => 'policies_third_party_liability'),
                       /*array(
                            'name'              => 'terms_years_id',
                        ...
                        array(
                            ...
                            'type'              => fldText,
                            'maxlenght'         => 20,
							'validationRule'	=> '402.[0-9][0-9].[0-9]{6}',
                            'display'           =>
                                array(
                                    'show'      => true,
                                    'insert'    => false,
    ...
    function Policies_ThirdPartyLiability($data) {
        Policies::Policies($data);

        $this->objectTitle = 'Policies_ThirdPartyLiability';

        $this->messages['plural'] = 'Договори "Добровільне страхування цивільної відповідальності перед третіми особами"';
        $this->messages['single'] = 'Договір "Добровільне страхування цивільної відповідальності перед третіми особами"';
	...
	function showForm($data, $action, $actionType=null, $template=null) {
		...
		$sql =	'SELECT a.id, a.title ' .
				...
				'WHERE a.product_types_id = ' . PRODUCT_TYPES_THIRD_PARTY_LIABILITY . ' ' .
				'ORDER BY a.title';
	...
	function setAdditionalFields($id, $data) {
        ...
        $sql =  'UPDATE ' . PREFIX . '_policies AS a ' .
                'JOIN ' . PREFIX . '_policies_third_party_liability AS b ON a.id = b.policies_id ' .
    ...
    function insert($data, $redirect=true, $showForm=true, $checkFieldsAndReturn=false) {
        ...
        if (intval($data['id'])) {
            ...
            if ($Authorization->data['roles_id'] == ROLES_AGENT) {
                ...
                $sql =  'UPDATE ' . PREFIX . '_policies_third_party_liability SET ' .
	...
	function renewPolicy($data) {//алгоритм Переукласти поліс
		...
		$data['product_types_id']	= PRODUCT_TYPES_THIRD_PARTY_ABILITY;
		$data['policy_statuses_id']	= POLICY_STATUSES_CREATED;
	...
	//Export 1C 7.7
     function getXML($data) {
      ...
      if ($data['number']) {
        ...
        return  $Smarty->fetch($this->object . '/third_party_ability.xml');
    }
