...
// +----------------------------------------------------------------------+
//
// $Id: common.php,v 1.72 2001/11/28 14:17:54 cox Exp $
//
...
class DB_common extends PEAR
{
    ...
    */
    ...
    function &getOne($query, $lifetime = 0, $params = array())
    {
    	if (MEMCACHE && $lifetime) {
	    	$row[0] = $this->Memcache->get(md5(DB_DATABASE . $query));
    	}

    	if (!MEMCACHE || !$lifetime || !$row[0]) {
    		//echo $query;
        	...
        	if (MEMCACHE) {
				$this->Memcache->set(md5(DB_DATABASE . $query), $row[0], false, $lifetime);
        	}
    	}
    ...
    {
    	if (MEMCACHE && $lifetime) {
	    	$row = $this->Memcache->get(md5(DB_DATABASE . $query));
    	}

    	if (!MEMCACHE || !$lifetime || !$row) {
    		//echo $query;
        	...
        	if (MEMCACHE) {
				$this->Memcache->set(md5(DB_DATABASE . $query), $row, false, $lifetime);
        	}
    	}
    ...
    function &getCol($query, $lifetime = 0, $col = 0, $params = array())
    {
    	if (MEMCACHE && $lifetime) {
	    	$ret = $this->Memcache->get(md5(DB_DATABASE . $query));
    	}

    	if (!MEMCACHE || !$lifetime || !$ret) {
    		//echo $query;
	        settype($params, 'array');
	        if (sizeof($params) > 0) {
    	        $sth = $this->prepare($query);
        	...
        	if (MEMCACHE) {
				$this->Memcache->set(md5(DB_DATABASE . $query), $ret, false, $lifetime);
	        }
    	}
    ...
    function &getAssoc($query, $lifetime = 0, $force_array = false, $params = array())
    {
    	if (MEMCACHE && $lifetime) {
	    	$results = $this->Memcache->get(md5(DB_DATABASE . $query));
    	}

    	if (!MEMCACHE || !$lifetime || !$results) {
    		//echo $query;
	        settype($params, 'array');
    	    if (sizeof($params) > 0) {
        	    $sth = $this->prepare($query);
        	...
        	if (MEMCACHE) {
				$this->Memcache->set(md5(DB_DATABASE . $query), $results, false, $lifetime);
	        }
    	}
