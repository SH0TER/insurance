...
echo '<table>
		<tr class="columns">
			<td>Дата</td>
			<td>банк новий</td>
			<td>банк пролонгація</td>
			<td>рітейл новий</td>
			<td>рітейл пролонгація</td>
			<td>укравто</td>
			<td>загалом</td>
		</tr>';

while (mktime(0, 0, 0, 8, 1, 2015) > mktime(0, 0, 0, $month, $day, $year)) {
    ...
    $sql = 'select a.id, a.policies_id, a.date, a.end_date, a.statuses_id, a.is_agr, b.number, a.number_insurance_year, a.number_prolongation, a.number_part_payment, c.financial_institutions_id, b.clients_id, c.insurer_edrpou
                ...
                where ' . $db->quote(date('Y-m-d', mktime(0, 0, 0, $month+1, $day, $year)-1)) . ' between a.date and a.end_date and a.valid = 1 and b.product_types_id = 3 and b.item <> \'Автопарк\' and datediff(a.end_date, a.date) <= 366
and year(a.date)=2014 and a.statuses_id>1
				';
    ...
    foreach ($list_first as $row_first) {
        if ($row_first['is_agr'] == 1) {
            ...
            if (in_array($row_first['clients_id'], $ukrauto_clients_id) || in_array($row_first['insurer_edrpou'], $ukrauto_edrpou)) $count_ukrauto++;
            elseif (intval($row_first['financial_institutions_id']) && intval($row_first['number_prolongation'])) $count_bank_prolong++;
            elseif (intval($row_first['financial_institutions_id'])) $count_bank_new++;
            elseif (intval($row_first['number_prolongation'])) $count_retail_prolong++;
            else $count_retail_new++;

            $count++;
        } elseif ($row_first['number_part_payment'] > 2) {
			$sql = 'select a.id, a.policies_id, a.date, a.end_date, a.statuses_id, a.is_agr, b.number, a.number_insurance_year, a.number_prolongation, c.financial_institutions_id, b.clients_id, c.insurer_edrpou
					from insurance_policy_payments_calendar a
					join insurance_policies b on a.policies_id = b.id
					join insurance_policies_kasko c on b.id = c.policies_id
    ...
    echo '<tr>';
    echo '<td align="left">' . date('Y-m-d', mktime(0, 0, 0, $month+1, $day, $year)-1) . '</td>';
