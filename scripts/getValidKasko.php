...
//_dump($last_payment);
        ...
        $sql = 'select e.title as regions_title, d.title as agencies_title, concat_ws(\'/\', c.brand, c.model) as item, concat_ws(\' \', b.insurer_lastname, b.insurer_firstname, b.insurer_patronymicname) as insurer,
                    getPolicyDate(a.number, 1) as policies_date, a.number as policies_number, b.financial_institutions_id, getPolicyDate(a.number, 2) as begin_p, getPolicyDate(a.number, 3) as end_p,
                    if(f.id > 0, f.item_price, c.car_price) as item_price, a.id as policies_id, b.options_agregate_no, c.year, c.engine_size, getCarParameters(3, c.car_engine_type_id) as car_engine_type_id, getCarParameters(1, c.car_body_id) as car_body_id,
					c.modification, getCarParameters(2, c.transmissions_id) as transmissions_id
	...
	if (!intval($item['options_agregate_no'])) {
		$item['item_price_other'] = $item['item_price'] - $item['payed_amount'];
	} else {
		$item['item_price_other'] = $item['item_price'];
	}
...
echo '<td>Пальне</td>';
echo '<td>Кузов</td>';
echo '<td>Трансмісія</td>';
echo '<td>Модифікація</td>';
echo '<td>Початок періоду</td>';
echo '<td>Кінець періоду</td>';
echo '<td>Початок договору</td>';
echo '<td>Кінець договору</td>';

echo '</tr>';
*/
foreach ($items as $item) {
	echo $item['policies_id'] . ',<br />';
/*
    echo '<tr>';

	echo '<td>' . $item['policies_id'] . '</td>';
	...
	echo '<td>' . $item['engine_size'] . '</td>';
	echo '<td>' . $item['car_engine_type_id'] . '</td>';
	echo '<td>' . $item['car_body_id'] . '</td>';
	echo '<td>' . $item['transmissions_id'] . '</td>';
	echo '<td>' . $item['modification'] . '</td>';
    echo '<td>' . date('d.m.Y', strtotime($item['begin'])) . '</td>';
    echo '<td>' . date('d.m.Y', strtotime($item['end'])) . '</td>';
	echo '<td>' . date('d.m.Y', strtotime($item['begin_p'])) . '</td>';
    echo '<td>' . date('d.m.Y', strtotime($item['end_p'])) . '</td>';
