...
//_dump(1)	;
	...
	$sql = 'select calendar.id as payments_calendar_id, policies.begin_datetime as policies_begin_datetime, policies.id as policies_id, policies_go.terms_id as terms_id, policies_go.shassi, policies_go.car_types_id as car_types_id, 
				...
				policies.number, policies.product_types_id, calendar.date as datte, policies.price, calendar.payment_date as payment_date_report, policies.agencies_id, policies.top 
	...
	if ($row['blank_series_parent'] == '' && ($row['terms_id'] == 25 || ($row['terms_id'] >= 19 && $row['car_types_id'] == 3)) && (strtotime($row['payment_date_report']) < strtotime('01.01.2017'))) {
		...
		if (strlen($item['shassi']) < 17 && $row['person_types_id'] == 1) {
			$sql = 'SELECT COUNT(*) ' .
...
//_dump($sql);
	...
	} elseif ($row['blank_series_parent'] == '' && ($row['terms_id'] == 25 || ($row['terms_id'] >= 19 && $row['car_types_id'] == 3))) {
		$sql = 'SELECT shassi, brands_id, models_id, IF(insurer_identification_code IS NULL, \'\', insurer_identification_code) as insurer_identification_code, IF(insurer_passport_series IS NULL, \'\', insurer_passport_series) as insurer_passport_series, IF(insurer_passport_number IS NULL, \'\', insurer_passport_number) as insurer_passport_number FROM insurance_policies_go WHERE policies_id = ' . intval($row['policies_id']);
		$item = $db->getRow($sql);

		if (strlen($item['shassi']) < 17 && $row['person_types_id'] == 1) {
			$sql =	'SELECT a.number_prolongation as num, a.id as id '.
					'FROM insurance_policy_payments_calendar a '.
					'JOIN insurance_policies as policies ON a.policies_id = policies.id '.
					'JOIN insurance_policies_go as policies_go ON policies.id = policies_go.policies_id '.
					'WHERE policies.insurance_companies_id = 4 AND policies_go.terms_id = 25 AND policies.payment_statuses_id > 1 AND policies_go.insurer_identification_code = '.
					$db->quote((strlen($item['insurer_identification_code']) ? $item['insurer_identification_code'] : '')).
					' AND policies.begin_datetime < ' . $db->quote($row['policies_begin_datetime']).
					' AND TIMESTAMPDIFF(MONTH, a.end_date, ' . $db->quote($row['datte']) . ') < 3 '.
					' AND policies_go.insurer_passport_series = ' . $db->quote((strlen($item['insurer_passport_series']) ? $item['insurer_passport_series'] : '')).
					' AND policies_go.insurer_passport_number = ' . $db->quote((strlen($item['insurer_passport_number']) ? $item['insurer_passport_number'] : '')).
					' AND policies_go.brands_id = ' . intval($item['brands_id']) . ' AND policies_go.models_id = ' . intval($item['models_id']) . ' GROUP BY begin_datetime ORDER BY begin_datetime DESC LIMIT 0, 1';

			$res = $db->getRow($sql);

			if(intval($res['id']) > 0) {
				$number_prolongation = intval($res['num']) + 1;
			} else {
				$number_prolongation = 0;
			}
			...
			$sql =	'SELECT a.number_prolongation as num, a.id as id '.
					...
					'JOIN insurance_policies as policies ON a.policies_id = policies.id '.
					'JOIN insurance_policies_go as policies_go ON policies.id = policies_go.policies_id ' .
			   		'WHERE policies.insurance_companies_id = 4 AND policies_go.terms_id = 25 AND policies.payment_statuses_id > 1'.
			   		' AND policies_go.shassi = ' . $db->quote($item['shassi']) .
			   		' AND TIMESTAMPDIFF(MONTH, a.end_date, ' . $db->quote($row['datte']) . ') < 3 '.
			   		' AND policies.begin_datetime < ' . $db->quote($row['policies_begin_datetime'])  . ' GROUP BY begin_datetime ORDER BY begin_datetime DESC LIMIT 0, 1';
...
$sql = 'select id, parent_id, date, begin_datetime, interrupt_datetime, product_types_id as product_types_id, policy_statuses_id
		from insurance_policies 
		where product_types_id = 3 and id=297093
		order by date';
//_dump($sql);
		...
		if (intval($policy['parent_id'])) {
			...
			while (true) {
				if ($policy['policy_statuses_id'] > 1) {
					$sql = 'update insurance_policy_payments_calendar set valid = 0 where second_fifty_fifty = 0 and policies_id = ' . intval($parent_id) . ' and (statuses_id=1 OR date >= ' . $db->quote($policy['begin_datetime']).')';
					//!!!тут колупался незнаю верно или нет
					//_dump($sql);
					$db->query($sql);
				}
