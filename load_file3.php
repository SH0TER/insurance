...
//mb_internal_encoding("UTF-8");
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			for ($i=1; $i<=$Excel->sheets[0]['numRows']; $i++) {
						...
						$sql='INSERT INTO temp1 (product,car_price1,car_price2,car_price3) '.
							 ' values('
							.$db->quote($Excel->sheets[0]['cells'][ $i ][1]).','.
							$db->quote($Excel->sheets[0]['cells'][ $i ][2]).',' .
							$db->quote($Excel->sheets[0]['cells'][ $i ][3]).','  .
							$db->quote($Excel->sheets[0]['cells'][ $i ][4]).' '   .
