...
<tr>
	...
	<td class="left right bottom top" colspan="7">
		особа, зазначена в Заяві на страхування, яка керує ТЗ на законних підставах {if $values.financial_institutions_id!=2}(без обмежень в залежності від стажу керування ТЗ та віку){/if}
	</td>
</tr>
</table><br />
...
{if $values.financial_institutions_id!=15}
<table cellpadding="5" cellspacing="0" width="100%">
<tr>
	<td width="20%"></td>
...
<tr>
	...
	<td>
		...
		<tr>
			...
			{if $values.terms_years_id==1 && ($values.paymentsCount==2 || $values.paymentsCount==4) && $values.agreement_types_id!=3}
				{if $smarty.section.roll.first}
				<td align="right" {if $smarty.section.roll.first || $values.paymentsCount==2 || $values.paymentsCount==4} rowspan="{$values.paymentsCount}"{/if}>{$values.yearsPayments[roll].item_price+$equipment_price|moneyformat:-1}</td>
				<td align="right" {if $smarty.section.roll.first || $values.paymentsCount==2 || $values.paymentsCount==4} rowspan="{$values.paymentsCount}"{/if}>{$values.yearsPayments[roll].rate_kasko}</td>
				{/if}
			{else}	
...
{if $values.financial_institutions_id!=44 && $values.financial_institutions_id!=58} 
...
<table cellpadding="5" cellspacing="0" width="100%">
    <tr>
        <td width="20%" rowspan="{if $values.financial_institutions_id==2}6{else}5{/if}" valign="top"><b>14. Додаткові опції</b></td>
...
<p>1.	Даних особливих умов страхування, підписаних Страховиком та Страхувальником (частина А Договору). Особливі умови страхування підписуються у {if $values.financial_institutions_id == 11 || $values.financial_institutions_id == 15}3 (трьох) {else}2 (двох){/if} примірниках, що мають однакову юридичну силу, по одному для Страховика та Страхувальника.
...
<tr>
	<td {if $values.financial_institutions_id == 11 || $values.financial_institutions_id == 15} width="33%" {else} width="48%" {/if} align="center"><b>СТРАХОВИК</b></td>
	<td {if $values.financial_institutions_id == 11 || $values.financial_institutions_id == 15} width="33%" {else} width="48%" {/if} align="center"><b>СТРАХУВАЛЬНИК</b></td>
	{if $values.financial_institutions_id == 11 || $values.financial_institutions_id == 15}
