                    ...
                    <tr>
                        <td>
                            <form id="<?=$this->objectTitle?>" name="<?=$this->objectTitle?>" action="<?=$_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="do" id="doval" value="<?=$this->object.'|generate'?>" />
								...
								<td >
									...
									if (is_array($data['inputparameters']))
									{
										...
										foreach($data['inputparameters'] as $parameter)
										{
											...
											echo $this->printParameter($data,$parameter);
                                            echo '</table>';
                                            $j++;
                                            if ($j==4) {echo '<br>';$j=0;}
							...
							if ($res)
							{
								...
								while($res->fetchInto($row)) {
									...
									foreach($data['outputparameters'] as $parameter)
									{
										...
										echo '<td>&nbsp;'.$this->printOutputParameter($row[$parameter['alias']],$parameter,$row).'</td>';
									}	
									echo '</tr>';
                                    $reccount++;
								}
