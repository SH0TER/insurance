<?
	...
	$profile_types = ProfileTypes::getList();
	
	$sql = 'SELECT a.id as id, CONCAT(a.lastname, \' \', a.firstname) as name ' .
		   'FROM ' . PREFIX . '_accounts as a ' .
		   'JOIN ' . PREFIX . '_account_groups_managers_assignments as b ON a.id = b.accounts_id ' .
		   'WHERE b.account_groups_id = 17 AND a.active = 1';
	$managers = $db->getAll($sql);

?>

<script>

    function checkSelectProfileTypesId() {
        if (parseInt($("select[name=profile_types_id] option:selected").val()) == 0) {
            alert("Оберіть тип анкети");
            return false;
        }
        return true;
    }
...
<div class="block">
	...
	<tr>
		...
		<td>
			...
			<input type="hidden" name="do" value="Profiles|show" />
			...
			<tr>
				<td height="28">
					...
					<tr>
						...
						<? if ($this->permissions['export']) echo '<td width="22"><a href="javascript: var b = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'Action10\'); if (b) b.click();" onMouseOver="var over = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'Action10\'); if (over) over.over(); return true;" onMouseOut="var out = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'Action10\'); if (out) out.out(); return true;"><img height="19" width="19" name="'.$this->objectTitle.'Action10" src="/images/administration/navigation/export.gif" alt="' . translate('Експорт Фалькон') . '" /></a></td>'?>
						<? if ($this->permissions['export']) echo '<td width="22"><a href="javascript: var b = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'Action11\'); if (b) b.click();" onMouseOver="var over = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'Action11\'); if (over) over.over(); return true;" onMouseOut="var out = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'Action11\'); if (out) out.out(); return true;"><img height="19" width="19" name="'.$this->objectTitle.'Action11" src="/images/administration/navigation/export.gif" alt="' . translate('Експорт Фалькон (DCSI sales)') . '" /></a></td>'?>
						<td width="10"></td>
				...
				<td align="right">
					<table cellpadding="0" cellspacing="5">
						<tr>
							<td><b>Тип:</b></td>
							<td>
								<select name="profile_types_id" class="fldSelect" onblur="this.className='fldSelect'" onfocus="this.className='fldSelectOver'">
									<?
										echo '<option value="0">...</option>';
										foreach ($profile_types as $profile_type) {
											echo '<option value="' . $profile_type['id'] . '" ' . ($profile_type['id'] == $data['profile_types_id'] ? 'selected' : '') . '>' . $profile_type['title'] . '</option>';
										}
									?>
								</select>
							</td>
							<td><b>Номер:</b></td>
							<td><input type="text" name="profiles_number" class="fldText" onfocus="this.className='fldTextOver';" onblur="this.className='fldText';" value="<?=$data['profiles_number']?>" /></td>
							<td><b>Створено:</b></td>
							...
							<td><b>Менеджер:</b></td>
							<td>
								<select name="managers_id" class="fldSelect" onblur="this.className='fldSelect'" onfocus="this.className='fldSelectOver'">
									<?
										echo '<option value="">...</option>';
										foreach ($managers as $manager) {
											echo '<option value="' . $manager['id'] . '" ' . ($manager['id'] == $data['managers_id'] ? 'selected' : '') . '>' . $manager['name'] . '</option>';
										}
									?>
								</select>
							</td>
			...
			<!--
				...
				<? if ($this->permissions['export']) echo 'document.'.$this->objectTitle.'.buttons[document.'.$this->objectTitle.'.buttons.length] = new MMCommandButton(\''.$this->objectTitle.'Action10\', document.'.$this->objectTitle.', \''.$this->object.'|exportFalconInWindow\', \'/images/administration/navigation/export.gif\', \'/images/administration/navigation/export_over.gif\', \'\', \'/images/administration/navigation/export_dim.gif\', true, true, true, true, \'' . translate('Експорт Фалькон') . '\', false, \'\');'?>
				<? if ($this->permissions['export']) echo 'document.'.$this->objectTitle.'.buttons[document.'.$this->objectTitle.'.buttons.length] = new MMCommandButton(\''.$this->objectTitle.'Action11\', document.'.$this->objectTitle.', \''.$this->object.'|exportFalconDCSIInWindow\', \'/images/administration/navigation/export.gif\', \'/images/administration/navigation/export_over.gif\', \'\', \'/images/administration/navigation/export_dim.gif\', true, true, true, true, \'' . translate('Експорт Фалькон (DCSI sales)') . '\', false, \'\');'?>
