    ...
    <body>
        <table border="1" cellpadding=0 cellspacing=0>
            ...
            <tr>
				...
				<td>
				<?
					$s = 'select a.number from insurance_bank_akts a join
					insurance_bank_akts_contents b on a.id = b.akts_id where b.id=' . $row['id'];
					echo $db->getOne($s);
				?>
				</td>
				<td>
				<?
					$s = 'select a.date from insurance_bank_akts a join
					insurance_bank_akts_contents b on a.id = b.akts_id where b.id=' . $row['id'];
					echo $db->getOne($s);
				?>
				</td>
				<td>
				<?
					$s = 'select c.title from insurance_bank_akts a join 
					insurance_bank_akts_contents b on a.id = b.akts_id 
					join insurance_payment_statuses c on a.payment_statuses_id=c.id 
					where b.id=' . $row['id'];
					echo $db->getOne($s);
				?>
				</td>
