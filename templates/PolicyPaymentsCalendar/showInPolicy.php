<script>
	...
	function changeTaskType(types_id) {
		$.ajax({
            type:		'POST',
            url:		'index.php',
            dataType:	'json',
            data:		'do=Tasks|getListStatusesInWindow&types_id=' + types_id,
            success: 	function(result) {
							$("select[name=task_status]").empty();
							$("select[name=task_status]").append('<option value=0>...</option>');
							for (i=0; i<result.length; i++) {
								if (parseInt(result[i].parent_id) > 0)
									$("select[name=task_status]").append('<option value=' + result[i].id + '>---' + result[i].title + '</option>');
								else
									$("select[name=task_status]").append('<option value=' + result[i].id + '>' + result[i].title + '</option>');
							}
                        }
	...
	function createTask() {
		$.ajax({
            ...
            url:		'index.php',
            dataType:	'json',
            data:		'do=Tasks|getListInWindow',
            success: 	function(result) {
							$("select[name=task_type]").append('<option value=0>...</option>');
							for (i=0; i<result.length; i++) {
								$("select[name=task_type]").append('<option value=' + result[i].id + '>' + result[i].title + '</option>');
							}
                        }
        });
		...
		$.ajax({
            ...
            url:		'index.php',
            dataType:	'json',
            data:		'do=Agents|getListInWindow&agencies_id=1469',
            success: 	function(result) {
							$("select[name=managers_id]").append('<option value=0>...</option>');
							for (i=0; i<result.length; i++) {
								$("select[name=managers_id]").append('<option value=' + result[i].id + '>' + result[i].name + '</option>');
							}
                        }
        });
		
		tb_show('<strong>Створити задачу:</strong>', '#TB_inline?height=150&width=450&inlineId=hiddenModalContentCreateTask', false);
	}
	
	function saveTask() {
		date = $('#task_date').val();
		if (parseInt($('select[name=task_type] option:selected').val()) > 0 && parseInt($('select[name=task_status] option:selected').val()) > 0 && parseInt($('select[name=managers_id] option:selected').val()) > 0 && checkdate1(date.substring(3, 5), date.substring(0, 2), date.substring(6, 10))) {
			calendar_id = $('form[name=<?=$this->objectTitle?>] input[name=id[]]:checked').val();
			$.ajax({
				type:		'POST',
				url:		'index.php',
				dataType:	'json',
				data:		'do=Tasks|createTaskInWindow' +
							'&policies_id=<?=$data['policies_id']?>' +
							'&calendar_id=' + calendar_id +
							'&date=' + $('#task_date').val() +
							'&task_types_id=' + $('select[name=task_type] option:selected').val() +
							'&task_statuses_id=' + $('select[name=task_status] option:selected').val() +
							'&performers_id=' + $('select[name=managers_id] option:selected').val(),
				success: 	function(result) {                            
								alert(result);
								tb_remove();
							}
	...
	<tr>
		...
		<td>
			...
			<tr>
				<td height="28">
					...
					<tr>
						...
						<? if ($this->permissions['createTask']) echo '<td width="22"><a href="javascript: if (createTask()) { var b = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'ActionCreateTask\'); if (b) b.click(); }" onMouseOver="var over = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'ActionCreateTask\'); if (over) over.over(); return true;" onMouseOut="var out = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'ActionCreateTask\'); if (out) out.out(); return true;"><img height="19" width="19" border="0" name="'.$this->objectTitle.'ActionCreateTask" src="/images/administration/navigation/add.gif" alt="Створити задачу" /></a></td>'?>
						<td width="10"></td>
			...
			<!--
				...
				<? if ($this->permissions['createTask']) echo 'document.'.$this->objectTitle.'.buttons[document.'.$this->objectTitle.'.buttons.length] = new MMCommandButton(\''.$this->objectTitle.'ActionCreateTask\', document.'.$this->objectTitle.', \''.$this->object.'|createTask\', \'/images/administration/navigation/add.gif\', \'/images/administration/navigation/add_over.gif\', \'\', \'/images/administration/navigation/add_dim.gif\', true, false, true, false, \'Створити задачу\', false, \'\');'?>
...
<div id="hiddenModalContentCreateTask" style="display:none">
	<input type="hidden" name="transfer_policies_id" />
	<input type="hidden" name="policy_payments_id" />
	<input type="hidden" name="transfer_max_amount" />
	<table cellpadding="5" cellspacing="0" style="text-align: right;">
		<tr>
			<td>Дата:</td><td><?=$this->getDateSelect(null, null, null, null, 'task_date')?></td>
		</tr>
		<tr>
			<td>Тип задачі:</td><td><select onchange="changeTaskType(this.value)" name="task_type"></select></td>
		</tr>
		<tr>
			<td>Статус:</td><td><select name="task_status"></select></td>
		</tr>
		<tr>
			<td>Виконавець:</td><td><select name="managers_id"></select></td>
		</tr>
		<tr>
			<td align="center" colspan="2"><input type="button" class="button" value="Зберегти" onClick="saveTask()"></td>
		</tr>	
	</table>
</div>