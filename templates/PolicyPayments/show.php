...
<script>
	...
	function checkChangeForPayment() {
		var length = $('form[name=<?=$this->objectTitle?>] input[name=id[]]:checked').length;
		var payments_id = 0;
	
		if (length > 1) {
			alert('Виберіть одну сплату');
			return false;
		} else if (length == 0) {
			return false;
		}
		payments_id = $('form[name=<?=$this->objectTitle?>] input[name=id[]]:checked').val();
		
		$.ajax({
            type:		'POST',
            url:		'index.php',
            dataType:	'json',
            data:		'do=PolicyPayments|changeForPaymentInWindow' +
            ...
            success: 	function(result) {                            
							if (result.status == 'error') {
								...
								$('input[name=transfer_policies_id]').val(result.transfer_policies_id);
								$('input[name=policy_payments_id]').val(result.policy_payments_id);
								$('input[name=transfer_max_amount]').val(result.transfer_max_amount);
								$('#transfer_policies_number_div').html(result.transfer_policies_number);
								$('#transfer_max_amount_div').html(result.transfer_max_amount);
								tb_show('<strong>Перекинути платіж:</strong>', '#TB_inline?height=120&width=360&inlineId=hiddenModalContentChangePayment', false);
							}
                        }
        });			
		return false;
	...
	function changeForPayment() {
		if (parseFloat($('input[name=transfer_amount]').val()) > parseFloat($('input[name=transfer_max_amount]').val())) {
			alert('Сума для перерахування не може перевищувати максимальну');
			return;
		} else if (parseFloat($('input[name=transfer_amount]').val()) == 0 || isNaN(parseFloat($('input[name=transfer_amount]').val()))) {
			alert('Сума для перерахування повинна бути більше 0');
			return;
		}
		...
		$.ajax({
            ...
            data:		'do=PolicyPayments|changeForPaymentInWindow' +
						'&policies_id=<?=$data['policies_id']?>' +
						'&transfer_policies_id=' + $('input[name=transfer_policies_id]').val() +
                        '&policy_payments_id=' + $('input[name=policy_payments_id]').val() +
						'&transfer_amount=' + $('input[name=transfer_amount]').val() +
                        '&check=0',
            success: 	function(result) {                            
							alert(result.message);
							tb_remove();
	...
	<tr>
		...
		<td>
			...
			<tr>
				<td height="28">
					...
					<tr>
						...
						<? if ($this->permissions['changeForPayment']) echo '<td width="22"><a href="javascript: if (checkChangeForPayment()) { var b = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'ActionChangeForPayment\'); if (b) b.click(); }" onMouseOver="var over = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'ActionChangeForPayment\'); if (over) over.over(); return true;" onMouseOut="var out = MM_getButtonWithName(document.'.$this->objectTitle.', \''.$this->objectTitle.'ActionChangeForPayment\'); if (out) out.out(); return true;"><img height="19" width="19" name="'.$this->objectTitle.'ActionChangeForPayment" src="/images/administration/navigation/change.gif" alt="Перекинути платіж" /></a></td>'?>
			...
			<!--
				...
				<? if ($this->permissions['changeForPayment']) echo 'document.'.$this->objectTitle.'.buttons[document.'.$this->objectTitle.'.buttons.length] = new MMCommandButton(\''.$this->objectTitle.'ActionChangeForPayment\', document.'.$this->objectTitle.', \''.$this->object.'|changeForPayment\', \'/images/administration/navigation/change.gif\', \'/images/administration/navigation/change_over.gif\', \'\', \'/images/administration/navigation/change_dim.gif\', true, false, true, false, \'Перекинути платіж\', false, \'\');'?>
...
<div id="hiddenModalContentChangePayment" style="display:none">
	...
	<table cellpadding="5" cellspacing="0" style="text-align: right;">
		<tr>
			<td>Кошти будуть зараховані на:</td><td><div id="transfer_policies_number_div"></div></td>
		</tr>
		<tr>
			<td>Максимальна сума перерахування, грн.:</td><td><div id="transfer_max_amount_div"></div></td>
		</tr>
		<tr>
			<td>Сума перерахування, грн.:</td><td><input type="edit" name="transfer_amount" /></td>
		</tr>
		<tr>
			<td align="center" colspan="2"><input type="button" class="button" value="Зберегти" onClick="changeForPayment()"></td>
		</tr>	
	</table>
</div>