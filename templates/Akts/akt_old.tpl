...
<tr>
	...
	(надалі - Агент), з іншої сторони, склали Акт про те, що за  {$row.aktdate}. Агентом відповідно 
...
ВЗ 1,5% згідно пп. 1.1 п. 16-1 підрозділу 10 розділу XX ПК.<br>
{/if}
...
{section name="rollfin" loop=$finmonitoring}
{if $smarty.section.rollfin.first || $finmonitoring[rollfin].break_page}
<div style="page-break-after: always"></div>

<!-- Блок по терористам-->
<h1 align=center>Лист ознайомлення з заходами по фінансовому моніторингу</h1>
<br><br><br><br>
<p>З наказом про внесення змін до Додатку №7 Правил проведення фінансового моніторингу, а саме до переліку осіб пов’язаних з здійсненням терористичної діяльності або стосовно яких застосовано міжнародні санкції на селекторному освітньо-практичному заході ознайомлений:
<br><br><br><br>

{/if}

<p>{$finmonitoring[rollfin].title}    {if $row.fop.title}{$row.fop.title}{else}{$row.lastname} {$row.firstname} {$row.patronymicname}{/if}  _______________________________ (підпис) <br><br><br><br>
{/section}
<div style="page-break-after: always"></div>
<!-- ******************************************-->
...
{if $data.person_types_id==31} <!-- для зама свои таблицы-->
{section name="roll" loop=$zamsales}
<br>Факт загальних продажів {$zamsales[roll].title} - {$zamsales[roll].fact}; Факт кредитних продажів ЭК {$zamsales[roll].title}- {$zamsales[roll].ek}
{if $zamsales[roll].ek>0}
<table border=1 cellspacing=0 cellpadding=5 width="100%">
<tr>
	<td width="50" align="center"  ><b>№ &nbsp;п/п</b></td>
	<td ><b>№ анкети</b></td>
...
{assign var="counter" value=1}
{section name="roll1" loop=$zamsales[roll].ekdetails}
<tr>
	<td width="50" align="center"  > {$counter}</td>
	<td >{$zamsales[roll].ekdetails[roll1].number} </td>
	<td  >{$zamsales[roll].ekdetails[roll1].carsTitle}</td>
	<td  >{$zamsales[roll].ekdetails[roll1].fio}</td>
