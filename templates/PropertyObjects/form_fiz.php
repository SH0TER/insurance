<script type="text/javascript">
    ...
    function addItem() {
		...
		selectTitle  += '    <option value="Земельна ділянка">Земельна ділянка</option>';
        selectTitle  += '</select>';
		...
		selectDeductible  += '    <option value="0.5">0.5</option>';
	...
	function setVisibility() {
		var values_id1 = $('#values_id1').val();
		if (values_id1=='41')
		{
			$('.ground_appointmentBlock').css('display', '');
			$('.land').css('display', 'none');
			$('#landtext').html('Об\'єкт страхування знаходиться в зонi');
			
		}
		else
		{
			$('.ground_appointmentBlock').css('display', 'none');
			$('.land').css('display', '');
			$('#landtext').html('Приватний будинок (квартира)<br />знаходиться в зоні');
		}
	}
	$(function() {
		$('#values_id1').bind('change', function() {setVisibility();} );
		setVisibility();
	});
</script>
<div class="block">
            ...
            <tr>
                <td>
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="object_location" value="<?=$data['object_location']?>" maxlength="255" class="fldText" onfocus="this.className='fldTextOver'" onblur="this.className='fldText'" <?=$this->getReadonly(false)?> /></td>
                    </tr>
                    <tr>
                        <td><?=$this->getMark()?>Об’єкт страхування:</td>
                        <td>
                            <?
                                $data['types_id'] = 1;
					...
					<tr class="ground_appointmentBlock"><? if (!$data['ground_appointment']) $data['ground_appointment']='житлова чи громадська забудова';?>
                        <td><?=$this->getMark()?>Цільове призначення <br>земельної ділянки:</td>
                        <td><input type="text" name="ground_appointment" value="<?=$data['ground_appointment']?>" maxlength="150" class="fldText" onfocus="this.className='fldTextOver'" onblur="this.className='fldText'" <?=$this->getReadonly(false)?> /></td>
                    </tr>
                    </table>
                    ...
                    <tr>
                        <td><?=$this->getMark()?><span id="landtext">Приватний будинок (квартира)<br />знаходиться в зоні</span>:</td>
                        <td>
                        <?
                            $data['types_id'] = 5;
                    ...
                    <tr style="background: <?=($i % 2 == 1) ? '#FFFFFF' : '#F0F0F0'?>">
                        <td>
                            <select id="items<?=$i?>title" idx="<?=$i?>" name="items[<?=$i?>][title]" class="fldSelect" onfocus="this.className=fldSelectOver" onblur="this.className=fldSelect" <?=$this->getReadonly(true)?>>
								...
								<option value="Земельна ділянка" <?=($item['title']=='Земельна ділянка' ? 'selected':'')?>>Земельна ділянка</option>
                            </select>
                        </td>
                        ...
                        <td>
                            <select id="items<?=$i?>value" name="items[<?=$i?>][value]" class="fldSelect" onfocus="this.className=fldSelectOver" onblur="this.className=fldSelect" <?=$this->getReadonly(true)?>>
								<option value="0.5" <?=($item['value']==1 ? 'selected':'')?>>0.5</option>
