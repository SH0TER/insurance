<script type="text/javascript">
	...
	function setInsurerIdCard() {
	    if($('[name=insurer_id_card]:checked').val() == 0) {
	        $('#passportMORTAGE').css('display', 'table');
	        $('table#insurer_new_passport_table').css('display', 'none');
	    } else {
	        $('#passportMORTAGE').css('display', 'none');
	...
	function changePersonType1(init) {
		switch ( $('select[name=assured_person_types_id] option:selected').val() ) {
			case '1'://физ. лицо
				$('.assuredpersonBlock').css('display', '');
				break;
			case '2'://юр. лицо
				$('.assuredpersonBlock').css('display', 'none');
				break;
		}
    }
	...
	if ($('input[id=types_id]').val()==2) return;
		...
		$.ajax({
			...
			data:		'do=Products|getRateInWindow' + 
						...
						'&fire_extinguishers_counts=' + $('input[name=fire_extinguishers_counts]').val() + 
						'&mortage_groups_id=' +  $('select[name=mortage_groups_id] option:selected').val() + 
						'&deductibles_value=' + $('select[name=deductibles_value] option:selected').val() + 
						'&terms_years_id=' + $('select[name=terms_years_id] option:selected').val() + 
    ...
    <table cellpadding="2" cellspacing="3" width="100%">
        <tr>
            <td>
				...
				<table class="personBlock" cellpadding="5" cellspacing="0" id="passportMORTAGE" <?= (intval($data['insurer_id_card'])?'style="display:none"':'') ?>>
					<tr>
				 ...
				 <table cellpadding="5" cellspacing="0" id="insurer_new_passport_table" <?= (intval($data['insurer_id_card'])?'':'style="display:none"') ?>>
	                <tr>
                ...
                <tr>
					<td class="label grey"><?=$this->getMark(false)?>Група застрахованого майна:</td>
					<td><?=$this->buildSelect($this->formDescription['fields'][ $this->getFieldPositionByName('mortage_groups_id') ], $data['mortage_groups_id' ], $data['languageCode'], '  onchange="calculate(false);" ' . $this->getReadonly(true), null, $data, $this->isEqual('mortage_groups_id'))?></td>
                    ...
                    <td align="left"><input <?if ($data['types_id']!=2) echo 'readonly' ?> type="text" onchange="calculate(false)" id="rate" name="rate" value="<?=$data['rate']?>" maxlength="10" class="fldMoney" onfocus="this.className='fldMoneyOver';" onblur="this.className='fldMoney';"   <?=$this->getReadonly(true)?>/></td>
				...
				<div class="section">Характеристика обєкта страхування:</div>
				 <table cellpadding="5" cellspacing="0">
				 <tr class="land">
                        <td><?=$this->getMark()?>Місце страхування (адреса місцезнаходження майна):</td>
                        <td>
							<input type="text"   name="mortage_place" value="<?=$data['mortage_place']?>" maxlength="255" class="fldText" onfocus="this.className='fldTextOver';" onblur="this.className='fldText';"   <?=$this->getReadonly(true)?>/>
							
                        </td>
                    </tr>
                     <tr class="land">
                        <td><?=$this->getMark()?>На страхування приймаються:</td>
                        <td>
                        <?
                            $data['block_id'] = 1;
                            echo $this->buildSelect($this->formDescription['fields'][$this->getFieldPositionByName('values_id')], $data['values_id'], null, ' onchange="calculate(false)" '.$this->getReadonly(true), null, $data)
                        ?>
                        </td>
                    </tr>
 
                     <tr class="land">
                        <td><?=$this->getMark()?>Регіон розташування застрахованого майна:</td>
                        <td>
                        <?
                            $data['block_id'] = 2;
                            echo $this->buildSelect($this->formDescription['fields'][$this->getFieldPositionByName('values_id')], $data['values_id'], null, ' onchange="calculate(false)" '.$this->getReadonly(true), null, $data)
					...
					<tr class="land">
                        <td><?=$this->getMark()?>Рік побудови застрахованого майна:</td>
                        <td>
							<input type="text"   onchange="calculate(false)" name="year" value="<?=$data['year']?>" maxlength="4" class="fldMoney" onfocus="this.className='fldMoneyOver';" onblur="this.className='fldMoney';"   <?=$this->getReadonly(true)?>/>
							 Наявнiсть та Кількість вогнегасників:
							<input type="text"   onchange="calculate(false)" name="fire_extinguishers_counts" value="<?=intval($data['fire_extinguishers_counts'])?>" maxlength="4" class="fldMoney" onfocus="this.className='fldMoneyOver';" onblur="this.className='fldMoney';"   <?=$this->getReadonly(true)?>/>
							(0- якщо вiтсутнi)
							
                        </td>
                    </tr>
					 <tr class="land">
                        <td><?=$this->getMark()?>Матеріал стін застрахованого майна:</td>
                        <td>
                        <?
                            $data['block_id'] = 3;
                            echo $this->buildSelect($this->formDescription['fields'][$this->getFieldPositionByName('values_id')], $data['values_id'], null, ' onchange="calculate(false)" '.$this->getReadonly(true), null, $data)
					 ...
					 <tr class="land">
                        <td><?=$this->getMark()?>Наявність автоматичної системи пожежогасіння:</td>
                        <td>
                        <?
                            $data['block_id'] = 4;
                            echo $this->buildSelect($this->formDescription['fields'][$this->getFieldPositionByName('values_id')], $data['values_id'], null, 'onchange="calculate(false)" multiple size="3" '.$this->getReadonly(true), null, $data)
                        ?>
                        </td>
                    </tr>
					 <tr class="land">
                        <td><?=$this->getMark()?>Захист від протиправних дій:</td>
                        <td>
                        <?
                            $data['block_id'] = 5;
                            echo $this->buildSelect($this->formDescription['fields'][$this->getFieldPositionByName('values_id')], $data['values_id'], null, ' onchange="calculate(false)" multiple size="3" '.$this->getReadonly(true), null, $data)
					 ...
					 <tr class="land">
                        <td><?=$this->getMark()?>Наявність засобів захисту:</td>
                        <td>
                        <?
                            $data['block_id'] = 6;
                            echo $this->buildSelect($this->formDescription['fields'][$this->getFieldPositionByName('values_id')], $data['values_id'], null, ' onchange="calculate(false)" '.$this->getReadonly(true), null, $data)
