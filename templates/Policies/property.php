<script type="text/javascript">
	...
	function setInsurerIdCard() {
	    if($('[name=insurer_id_card]:checked').val() == 0) {
	        $('#passportPROPERTY').show();
	        $('table#insurer_new_passport_table').hide();
	    } else {
	        $('#passportPROPERTY').hide();
    ...
    function setAssured(obj) {
        $('#assuredBlock2').css('display', (obj.checked) ? 'block' : 'none');
    }

	function changeFinancialInstitution() {
		if ($('select[name=financial_institutions_id] option:selected').val() > 0) {
			$('#assured').attr('checked', false);
			$('#assuredBlock1').css('display', 'none');
			$('#assuredBlock2').css('display', 'none');
		} else {
			$('#assuredBlock1').css('display', 'block');
		}
	}
	...
	{
		if ($('select[name=insurer_person_types_id] option:selected').val() !='1') return;
		
		if ($('#multi_year').attr('checked'))
		{
			$('#end_datetime').attr('readonly', false);
			$('#end_datetime').attr('style', '');
		}
    ...
    function setEnd() {
        ...
        if ( $('select[name=insurer_person_types_id] option:selected').val() != '1') return;
		if ($('#multi_year').attr('checked')) return;
        ...
        with (document.<?=$this->objectTitle?>) {
            ...
            if (beginDay>0 && beginMonth>0 && beginYear>0) {
                ...
                switch ( $('select[name=terms_id] option:selected').val() ) {
					case '1'://1 місяць
						addmonth = 1;
						break;
					case '2'://2 місяці
    ...
    <table cellpadding="2" cellspacing="3" width="100%">
        <tr>
            <td>
				...
				<table class="companyBlock" cellpadding="5" cellspacing="0">
				<tr>
					<td class="label grey"><?=$this->getMark()?>Вид діяльності організації:</td>
					<td><?=$this->buildSelect($this->formDescription['fields'][ $this->getFieldPositionByName('organization_types_id') ], $data['organization_types_id'], $data['languageCode'], ' ' . $this->getReadonly(true), null, $data, $this->isEqual('organization_types_id'))?></td>
					<td class="label grey organizationTitleBlock">Інше:</td>
					<td class="organizationTitleBlock"><input type="text" name="organization_types_title" value="<?=$data['organization_types_title']?>" class="fldText company<?=$this->isEqual('organization_types_title')?>" onfocus="this.className='fldTextOver company<?=$this->isEqual('organization_types_title')?>'" onblur="this.className='fldText company<?=$this->isEqual('organization_types_title')?>'" <?=$this->getReadonly(false)?> /></td>
				</tr>
				</table>
				...
				<table cellpadding="5" cellspacing="0" class="companyBlock">
				<tr>
					<td class="label grey"><?=$this->getMark()?>Прізвище родовий:</td>
					<td><input type="text" name="insurer_lastname_rod" value="<?=$data['insurer_lastname_rod']?>" maxlength="50" class="fldText lastname<?=$this->isEqual('insurer_lastname_rod')?>" onfocus="this.className='fldTextOver lastname<?=$this->isEqual('insurer_lastname_rod')?>'" onblur="this.className='fldText lastname<?=$this->isEqual('insurer_lastname_rod')?>'" <?=$this->getReadonly(false)?> /></td>
					<td class="label grey"><?=$this->getMark()?>Ім'я родовий:</td>
					<td><input type="text" name="insurer_firstname_rod" value="<?=$data['insurer_firstname_rod']?>" maxlength="50" class="fldText firstname<?=$this->isEqual('insurer_firstname_rod')?>" onfocus="this.className='fldTextOver firstname<?=$this->isEqual('insurer_firstname_rod')?>'" onblur="this.className='fldText firstname<?=$this->isEqual('insurer_firstname_rod')?>'" <?=$this->getReadonly(false)?> /></td>
					<td class="label grey"><?=$this->getMark()?>По батькові родовий:</td>
					<td><input type="text" name="insurer_patronymicname_rod" value="<?=$data['insurer_patronymicname_rod']?>" maxlength="50" class="fldText patronymicname<?=$this->isEqual('insurer_patronymicname_rod')?>" onfocus="this.className='fldTextOver patronymicname<?=$this->isEqual('insurer_patronymicname_rod')?>'" onblur="this.className='fldText patronymicname<?=$this->isEqual('insurer_patronymicname_rod')?>'" <?=$this->getReadonly(false)?> /></td>
				...
				<table class="personBlock" cellpadding="5" cellspacing="0" id="passportPROPERTY" <?= (intval($data['insurer_id_card'])?'style="display:none"':'') ?>>
				<tr>
				...
				<tr>
					...
					<td class="companyBlock"><input type="text" name="insurer_position_rod" value="<?=$data['insurer_position_rod']?>" maxlength="150" class="fldText place<?=$this->isEqual('insurer_position_rod')?>" onfocus="this.className='fldTextOver place<?=$this->isEqual('insurer_position_rod')?>'" onblur="this.className='fldText place<?=$this->isEqual('insurer_position_rod')?>'" <?=$this->getReadonly(false)?> /></td>
				...
				<tr>
					<td colspan="8">
						...
						<tr>
							...
							<td><input type="text" name="credit_agreement_number" value="<?=$data['credit_agreement_number']?>" maxlength="16" class="fldText number<?=$this->isEqual('credit_agreement_number')?>" onfocus="this.className='fldTextOver number<?=$this->isEqual('credit_agreement_number')?>'" onblur="this.className='fldText number<?=$this->isEqual('credit_agreement_number')?>'" <?=$this->getReadonly(false)?> /></td>
						...
						<tr>
							...
							<td><input type="text" name="pawn_agreement_number" value="<?=$data['pawn_agreement_number']?>" maxlength="20" class="fldText number<?=$this->isEqual('pawn_agreement_number')?>" onfocus="this.className='fldTextOver number<?=$this->isEqual('pawn_agreement_number')?>'" onblur="this.className='fldText number<?=$this->isEqual('pawn_agreement_number')?>'" <?=$this->getReadonly(false)?> /></td>
							<td class="label grey">від</td>
				...
				<tr>
					...
					<td><input type="text" name="zones_title" value="<?=$data['zones_title']?>" class="fldText company<?=$this->isEqual('zones_title')?>" onfocus="this.className='fldTextOver company<?=$this->isEqual('zones_title')?>'" onblur="this.className='fldText company<?=$this->isEqual('zones_title')?>'" <?=$this->getReadonly(false)?> /></td>
				</tr>
				</table>
				...
				<tr>
					...
					<? } ?>
                    <td class="label grey terms_id"><?=$this->getMark(false)?>Термін страхування:</td>
				    <td class="terms_id"><?=$this->buildSelect($this->formDescription['fields'][ $this->getFieldPositionByName('terms_id') ], $data['terms_id' ], $data['languageCode'], 'onchange="setEnd(); " ' . $this->getReadonly(true), null, $data, $this->isEqual('terms_id'))?></td>
					<td>Багатолiтнiй:</td>
					<td><input type="checkbox" id="multi_year" name="multi_year" value="1" <?=($data['multi_year']) ? 'checked' : ''?> <?=$this->getReadonly(true)?> onClick="setMultiple()"/></td>
