<script type="text/javascript">
	...
	function setInsurerIdCard() {
    	if($('[name=insurer_id_card]:checked').val() == 0) {
    		$('table#passportONESHIPPING').show();
    		$('table#insurer_new_passport_table').hide();
    	} else {
    		$('table#passportONESHIPPING').hide();
	...
	if ($('input[id=types_id]').val()==2) return;
		...
		$.ajax({
			...
			data:		'do=Products|getRateInWindow' + 
						...
						'&deductibles_value=' + $('#deductible').val() + 
						'&rate=' + $('#rate').val() ,
    ...
    <table cellpadding="2" cellspacing="3" width="100%">
        <tr>
			...
			<table cellpadding="5" cellspacing="0" id="insurer_id_card_table">
				...
				<table class="personBlock" cellpadding="5" cellspacing="0" id="passportONESHIPPING" <?= (intval($data['insurer_id_card'])?'style="display:none"':'') ?>>
				<tr>
				...
				<table id="assuredBlock2" cellpadding="0" cellspacing="5">
				<tr>
					<td class="label grey"><?=$this->getMark(false)?>Назва:</td>
					...
					<td class="label grey"><?=$this->getMark(false)?>ЄРДПОУ:</td>
                ...
                <tr>
					...
					<td class="label grey"><b>Франшиза, %:</b></td>
                    <td align="left"><input type="text"   id="deductible" name="rate" value="<?=$data['deductible']?>" maxlength="10" class="fldMoney" onfocus="this.className='fldMoneyOver';" onblur="this.className='fldMoney';"   <?=$this->getReadonly(true)?>/></td>
				...
				<tr>
					<td class="label grey"><?=$this->getMark(false)?>Місце завантаження (адреса):</td>
					<td><input type="text" name="place1" value="<?=$data['place1']?>" maxlength="150" class="fldText address<?=$this->isEqual('place1')?>" onfocus="this.className='fldTextOver address<?=$this->isEqual('place1')?>'" onblur="this.className='fldText address<?=$this->isEqual('place1')?>'" <?=$this->getReadonly(false)?> /></td>
					<td class="label grey"><?=$this->getMark(false)?>Місце розвантаження (адреса):</td>
					<td><input type="text" name="place2" value="<?=$data['place2']?>" maxlength="150" class="fldText address<?=$this->isEqual('place1')?>" onfocus="this.className='fldTextOver address<?=$this->isEqual('place1')?>'" onblur="this.className='fldText address<?=$this->isEqual('place1')?>'" <?=$this->getReadonly(false)?> /></td>
					<td class="label grey"><?=$this->getMark(false)?>Місця перевантаження, проміжного зберігання, складування:</td>
					<td><input type="text" name="place3" value="<?=$data['place3']?>" maxlength="150" class="fldText address<?=$this->isEqual('assured_bank_mfo')?>" onfocus="this.className='fldTextOver address<?=$this->isEqual('assured_bank_mfo')?>'" onblur="this.className='fldText address<?=$this->isEqual('assured_bank_mfo')?>'" <?=$this->getReadonly(false)?> /></td>
				</tr>
				<tr>
					<td class="label grey"><?=$this->getMark(false)?>Найменування вантажу:</td>
					<td><input type="text" name="cargo_name" value="<?=$data['cargo_name']?>" maxlength="150" class="fldText address<?=$this->isEqual('cargo_name')?>" onfocus="this.className='fldTextOver address<?=$this->isEqual('cargo_name')?>'" onblur="this.className='fldText address<?=$this->isEqual('cargo_name')?>'" <?=$this->getReadonly(false)?> /></td>
					<td class="label grey"><?=$this->getMark(false)?>Кількість місць:</td>
					<td><input type="text" name="place_count" value="<?=$data['place_count']?>" maxlength="15" class="fldMoney<?=$this->isEqual('place_count')?>" onfocus="this.className='fldMoney<?=$this->isEqual('place_count')?>'" onblur="this.className='fldMoney<?=$this->isEqual('place_count')?>'" <?=$this->getReadonly(false)?> /></td>
					<td colspan=2></td>
				</tr>
				<tr>
					<td class="label grey"><?=$this->getMark(false)?>Засіб перевезення :</td>
					<td><input type="text" name="tool_carriage" value="<?=$data['tool_carriage']?>" maxlength="150" class="fldText address<?=$this->isEqual('tool_carriage')?>" onfocus="this.className='fldTextOver address<?=$this->isEqual('tool_carriage')?>'" onblur="this.className='fldText address<?=$this->isEqual('tool_carriage')?>'" <?=$this->getReadonly(false)?> /></td>
					<td class="label grey"><?=$this->getMark(false)?>Упаковка, кріплення :</td>
					<td><input type="text" name="packaging" value="<?=$data['packaging']?>" maxlength="150" class="fldText address<?=$this->isEqual('packaging')?>" onfocus="this.className='fldTextOver address<?=$this->isEqual('packaging')?>'" onblur="this.className='fldText address<?=$this->isEqual('packaging')?>'" <?=$this->getReadonly(false)?> /></td>
					<td class="label grey"><?=$this->getMark(false)?>Транспорті документи:</td>
					<td><input type="text" name="documents_transportation" value="<?=$data['documents_transportation']?>" maxlength="150" class="fldText address<?=$this->isEqual('documents_transportation')?>" onfocus="this.className='fldTextOver address<?=$this->isEqual('documents_transportation')?>'" onblur="this.className='fldText address<?=$this->isEqual('documents_transportation')?>'" <?=$this->getReadonly(false)?> /></td>
				</tr>
				</table>
				...
				<table   cellpadding="0" cellspacing="5">
				<tr>
                        <td><?=$this->getMark()?><span id="landtext">Вид транспорту, яким перевозиться вантаж:</td>
                        <td>
                        <?
                            $data['types_id'] = 9;
				...
				<table   cellpadding="0" cellspacing="5">
				<tr>
                        <td><?=$this->getMark()?><span id="landtext">Умова страхування :</td>
                        <td>
                        <?
                            $data['types_id'] = 10;
                            echo $this->buildSelect($this->formDescription['fields'][$this->getFieldPositionByName('values_id')], $data['values_id'], null, 'multiple size="7" '. $this->getReadonly(true), null, $data)
