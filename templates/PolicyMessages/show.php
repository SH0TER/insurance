<? if (!intval($data['offset' . $this->objectTitle . 'Block']) && false) { ?>
<script type="text/javascript" src="js/jquery/spy.js"></script>
<script type="text/javascript">

	$(document).ready(function() {
		$('#<?=$this->object . 'Content'?>').spy({ limit: <?=$Authorization->data['records_per_page']?>, ajax: '/index.php?do=QuestionnaireMessages|getLastInWindow&questionnairesId=<?=$data['questionnairesId']?>&solutions_id=<?=$data['solutions_id']?>', isDupes : check_for_dupe, timeout: 120000});
	});
	...
	<tr>
		...
		<td>
			...
			<tr>
				<td>
					<? if ($total) {?>
						<table id="<?=$this->object . 'Content'?>" width="100%" cellpadding="0" cellspacing="0">
						<tr class="columns">
						...
						<tr class="<?=$this->getRowClass($row, $i)?>">
							<td><input type="checkbox" name="id[]" value="<?=$row['id']?>" onclick="javascript: MM_toggleItem(document.<?=$this->objectTitle?>, 'id[]')"></td>
