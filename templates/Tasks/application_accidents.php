...
<tr>
    <td></td>
    <td><? $this->getInfo($data)/*include_once 'Tasks/application_accidents_info.php'*/?></td>
</tr>
<tr>
    <td></td>
    <td>
            ...
            <tr>
                <td id="QuestionnaireMessageInWindow">
                    ...
                    <form id="<?=$this->objectTitle?>form" name="<?=$this->objectTitle?>" action="<?=$_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="task_statuses_call_id" value="<?=$data['task_statuses_call_id']?>" />
                        <table>
<!--
                        <tr>
...
-->
						...
						<tr>
                            <td colspan="2"><b>Чи подано заяву про подію:</b></td>
                            <td width="150">
								<input type="radio" name="app" value="1" <?=($data['app'] == 1 ? 'checked' : '')?> />так 
								<input type="radio" name="app" value="-1" <?=($data['app'] == -1 ? 'checked' : '')?> />ні
							</td>
                            <td colspan="2">&nbsp;</td>
                        </tr>
                        <tr>
                            <td colspan="2"><b>Дата звернення на СТО:</b></td>
                            <td width="150"><?=$this->getDateSelect(0, $data['app_date_year'], $data['app_date_month'], $data['app_date_day'], 'app_date', $this->getReadonly(true))?></td>
                            <td colspan="2">&nbsp;</td>
                        </tr>
						<tr>
                            <td colspan="2"><b>Чи запрошували на СТО:</b></td>
                            <td width="150">
								<input type="radio" name="sto" value="1" <?=($data['sto'] == 1 ? 'checked' : '')?> />так 
								<input type="radio" name="sto" value="-1" <?=($data['sto'] == -1 ? 'checked' : '')?> />ні
							</td>
                            <td colspan="2">&nbsp;</td>
                        </tr>
                        <tr>
