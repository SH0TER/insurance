...
<form name="<?=$this->objectTitle?>" action="<?=$_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data">
	...
	<input type="hidden" name="product_types_id" value="<?=PRODUCT_TYPES_PROPERTY?>" />
	...
	<tr>
		...
		<td><input type="text" name="amount_rough" value="<?=($data['amount_rough'] > 0) ? $data['amount_rough'] : ''?>" class="fldMoney" onfocus="this.className='fldMoneyOver'" onblur="this.className='fldMoney'" <?=$this->getReadonly()?> /></td>
