...
class FileUpload {
	...
	function checkFileName($fileName) {
		...
		if (strlen($fileName)>$this->fckphp_config['MaxFileNameLength']) return false;

		//Check that it only contains valid characters
		for($i=0;$i<strlen($fileName);$i++) if (!in_array(substr($fileName,$i,1),$this->fckphp_config['FileNameAllowedChars'])) return false;
...
// 			$_FILES['NewFile']['name']=$_FILES['NewFile']['name']; //$this->cleanFilename($_FILES['NewFile']['name']);
		...
		if (sizeof($_FILES)>0) {
			...
			if ($this->checkFileName($_FILES['NewFile']['name'])) {
						...
						if (in_array(strtolower($ext),$typeconfig['AllowedExtensions'])) {
							...
							if ($this->fckphp_config['DiskQuota']['Global']!=-1) {
								foreach ($this->fckphp_config['ResourceTypes'] as $resType) {
									...
									if ($dirSizes[$resType]===false && false) {
